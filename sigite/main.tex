...
Periodically, we reviewed the codebook itself together at two levels. First, we would look at the codes themselves (as opposed to the sentences) for relationships (`axial coding'). Second, we looked at all the samples within each specific code to see if they were coherent and to further sub-divided as necessary (`code upon'). We re-coded any sentences that stuck out as appearing different (should the sentence be coded as something else? Does it require a new code we do not have? Is it actually coded correctly but sticking out for some other reason?).
